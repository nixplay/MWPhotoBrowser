...
- (void)updateNavigation {
    ...
    if (_gridController) {
        ...
        if (_gridController.selectionMode) {
            self.title = NSLocalizedStringFromTableInBundle(@"Select Photos", @"MWPhotoBrowser", bundle, @"Select Photos");
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedStringFromTableInBundle(@"photo", @"MWPhotoBrowser", bundle, @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedStringFromTableInBundle(@"photos", @"MWPhotoBrowser" , bundle, @"Used in the context: '3 photos'");
            }
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            self.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedStringFromTableInBundle(@"of", @"MWPhotoBrowser" , bundle, @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
    } else {
        self.title = nil;
    }
